...
const ChatInterface = () => {
  ...
  const buttons = [
    { label: "I'm unemployed and need to pay bills.", message: "I'm unemployed and need to pay bills." },
    { label: "I need financial assistance.", message: "I need financial support." },
    { label: "I'm looking to get assistive technology.", message: "I'm looking to get assistive technology" },
    // ... add other buttons as needed
  ];

  const handleButtonClick = async (buttonMessage) => {
    const userMessage = {
      text: buttonMessage,
      sender: 'user'
    };
    ...
    try {
      const response = await axios.post('/api/message', { message: buttonMessage });
      const botMessage = {
        text: response.data,
        sender: 'bot'
      };
    ...
    <div className="chat-container">
      ...
      <div className="button-group">
        {buttons.map((button, index) => (
          <button key={index} onClick={() => handleButtonClick(button.message)} disabled={isLoading}>
            {button.label}
          </button>
        ))}
      </div>
